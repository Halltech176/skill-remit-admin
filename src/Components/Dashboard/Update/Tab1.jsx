...
const Tab1 = () => {
  ...
  const renderSkills = skills.map((data, index) => {
    return (
      <div
        ...
        className="mr-5 w-40 font-dm rounded-xl p-3 relative"
      >
        <p>{data}</p>
  ...
  return (
    <>
      <form className="flex mx-10 my-5" action="">
        <input
          placeholder="Enter New Skill"
          className="bg-primary-100 p-3 text-normal font-aeonik font-semibold text-md rounded-md w-96"
          type="text"
        />
        <button className="btn w-44 mx-48">Add</button>
      </form>
      <div className="flex items-center my-10">{renderSkills}</div>
    </>
  );
};
